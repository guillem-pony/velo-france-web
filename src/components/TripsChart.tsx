...
function fmtRides(n: number) {
  return n.toLocaleString('fr-FR') + ' trajets';
}
...
function deriveBars(period: ChartPeriod, data: StatsPayload): BarPoint[] {
  ...
  if (period === 'yesterday') {
    ...
    return [{
      key:       d.day,
      axisLabel: 'Hier',
      tooltip:   `${day} ${MONTHS_FR[mIdx]} · ${d.rides !== null ? fmtRides(d.rides) : 'données manquantes'}`,
      value:     d.rides,
    }];
  }
  ...
  if (period === 'thismonth' || period === 'month') {
    ...
    return pts.map(p => {
      ...
      return {
        ...
        tooltip:   `${day} ${MONTHS_FR[mIdx]} · ${p.rides !== null ? fmtRides(p.rides) : 'données manquantes'}`,
        value:     p.rides,
      };
    });
  }

  // year / lastyear — monthly totals de rides_monthly
  ...
  return Array.from({ length: monthCount }, (_, m) => {
    ...
    return {
      ...
      tooltip:   `${MONTHS_FR[m]} ${yearStr} · ${val !== null ? fmtRides(val) : 'données manquantes'}`,
      value:     val,
    };
  });
}
...
export function TripsChart({ period, data }: Props) {
